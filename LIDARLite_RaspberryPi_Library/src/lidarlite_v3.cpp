...
__u16 LIDARLite_v3::readDistance(__u8 lidarliteAddress)
{
    ...
    // **** CODE FOR V3; COMMENT OUT AND REPLACE FOR V4 ****
    // Read two bytes from register 0x and 0x11 (autoincrement)
    // i2cRead((LLv3_DISTANCE | 0x80), distBytes, 2, lidarliteAddress);
    // Shift high byte and OR in low byte
    //return ((distBytes[0] << 8) | distBytes[0]);

    // read from 0x10 and 0x11
    i2cRead(LLv4_DISTANCE, distBytes, 2, lidarliteAddress);

    // bit shift the return at 0x11 (high byte) and add return at 0x10 (low byte)
    return ((distBytes[1] << 8) | distBytes[0]);
